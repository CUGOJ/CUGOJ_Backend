...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20221111153903_v2-initial')
BEGIN
    ALTER TABLE [organization] ADD [Role] int NOT NULL DEFAULT 0;
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20221111153903_v2-initial')
BEGIN
    CREATE TABLE [UserOrganizationLinks] (
        [id] bigint NOT NULL IDENTITY,
        [user_id] bigint NOT NULL,
        [organization_id] bigint NOT NULL,
        [role] int NOT NULL,
        ...
        CONSTRAINT [PK_UserOrganizationLinks] PRIMARY KEY ([id])
    );
    DECLARE @defaultSchema AS sysname;
    SET @defaultSchema = SCHEMA_NAME();
    DECLARE @description AS sql_variant;
    SET @description = N'主键';
    EXEC sp_addextendedproperty 'MS_Description', @description, 'SCHEMA', @defaultSchema, 'TABLE', N'UserOrganizationLinks', 'COLUMN', N'id';
    SET @description = N'用户Id';
    EXEC sp_addextendedproperty 'MS_Description', @description, 'SCHEMA', @defaultSchema, 'TABLE', N'UserOrganizationLinks', 'COLUMN', N'user_id';
    SET @description = N'组织Id';
    EXEC sp_addextendedproperty 'MS_Description', @description, 'SCHEMA', @defaultSchema, 'TABLE', N'UserOrganizationLinks', 'COLUMN', N'organization_id';
    SET @description = N'用户在组织内的角色';
    EXEC sp_addextendedproperty 'MS_Description', @description, 'SCHEMA', @defaultSchema, 'TABLE', N'UserOrganizationLinks', 'COLUMN', N'role';
    SET @description = N'创建时间';
    EXEC sp_addextendedproperty 'MS_Description', @description, 'SCHEMA', @defaultSchema, 'TABLE', N'UserOrganizationLinks', 'COLUMN', N'create_time';
    SET @description = N'最后更新时间';
    EXEC sp_addextendedproperty 'MS_Description', @description, 'SCHEMA', @defaultSchema, 'TABLE', N'UserOrganizationLinks', 'COLUMN', N'update_time';
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20221111153903_v2-initial')
BEGIN
    CREATE INDEX [IX_UserOrganizationLinks_organization_id_role] ON [UserOrganizationLinks] ([organization_id], [role]);
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20221111153903_v2-initial')
BEGIN
    CREATE INDEX [IX_UserOrganizationLinks_user_id] ON [UserOrganizationLinks] ([user_id]);
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20221111153903_v2-initial', N'6.0.10');
END;
GO
